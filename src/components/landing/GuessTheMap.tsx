...
function GameWaitlistCTA() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
      <p className="text-sm text-[#ECE8E1]/50 mb-3">Know your maps? Track your real map win rates.</p>
      {done ? (
        ...
        <form onSubmit={submit} className="flex items-center gap-2 max-w-sm mx-auto">
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" className="flex-1 px-3 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-[#ECE8E1] placeholder-[#ECE8E1]/30 focus:border-[#14B8A6]/50 focus:outline-none" />
          <button type="submit" disabled={loading} className="px-4 py-2.5 rounded-lg bg-[#14B8A6] hover:bg-[#0D9488] text-white font-oswald font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50">{loading ? '...' : 'Join'}</button>
        </form>
      )}
    </motion.div>
  );
